...
class JunitNumLoopStatementsTest {
	...
	void test1() throws IOException, CheckstyleException {
		...
		File file = new File(filePath + "NumLoopStatementsTest1.java");
	...
	void test2() throws IOException, CheckstyleException {
		// Build File
		String filePath = "src/test/java/CategoryBTestFiles/";
		File file = new File(filePath + "NumLoopStatementsTest2.java");
		FileText ft = new FileText(file,"UTF-8");
		FileContents fc = new FileContents(ft);
	...
	public void helper(AbstractCheck b, DetailAST a) {
		while(a != null) {
			if (a.getType() == TokenTypes.LITERAL_FOR || 
				a.getType() == TokenTypes.LITERAL_WHILE ||
				a.getType() == TokenTypes.DO_WHILE) {
					results++;
			}
